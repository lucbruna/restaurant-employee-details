...
export default function CustomersPage() {
  ...
  const saveCustomer = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(editingCustomer ? "Falha ao atualizar cliente" : "Falha ao adicionar cliente");
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6 p-6">
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              <div className="space-y-3">
                <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-[11px] font-black uppercase tracking-[0.24em] text-primary">
                  ...
                  Livro de Visitas Bhukkad
                </div>
                <div className="space-y-2">
                  <h1 className="brand-display text-3xl font-semibold tracking-tight text-text-primary sm:text-4xl">
                    Perfis de clientes com contexto de serviço mais limpo
                  </h1>
                  <p className="max-w-3xl text-sm leading-7 text-text-secondary sm:text-base">
                    Mantenha histórico de contato, perfis vinculados ao GST e consulta de clientes recorrentes
                    alinhados com o resto do painel operacional.
                  </p>
                </div>
              </div>

              <div className="grid gap-3 sm:grid-cols-3">
                <SummaryTile label="Perfis" value={customers.length.toString()} />
                <SummaryTile label="Visíveis" value={filteredCustomers.length.toString()} />
                <SummaryTile label="Vinculados GST" value={customersWithGst.toString()} />
              </div>
            </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/40 hover:bg-muted/40">
                <TableHead className="text-[11px] font-black uppercase tracking-[0.18em] text-text-muted">Cliente</TableHead>
                <TableHead className="text-[11px] font-black uppercase tracking-[0.18em] text-text-muted">Contato</TableHead>
                <TableHead className="text-[11px] font-black uppercase tracking-[0.18em] text-text-muted">Total de Pedidos</TableHead>
                <TableHead className="text-[11px] font-black uppercase tracking-[0.18em] text-text-muted">Total Gasto</TableHead>
                <TableHead className="text-[11px] font-black uppercase tracking-[0.18em] text-text-muted">Última Visita</TableHead>
                  ...
                  <TableRow key={customer.id} className="hover:bg-muted/20 transition-colors">
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="font-medium text-text-primary">{customer.name || "Desconhecido"}</div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="text-sm">
                        <div className="font-medium text-text-primary">{customer.phone}</div>
                        <div className="text-text-muted">{customer.email || "Nenhum email adicionado"}</div>
                      </div>
                    </TableCell>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="rounded-[var(--radius-xxl)] border-border/70 bg-card shadow-[var(--shadow-elevation-3)]">
            <DialogHeader>
              <DialogTitle className="text-text-primary">{editingCustomer ? "Editar Cliente" : "Adicionar Cliente"}</DialogTitle>
            </DialogHeader>
            ...
            <form className="space-y-4" onSubmit={saveCustomer}>
              ...
              <textarea
                placeholder="Endereço ou observações"
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button type="submit" disabled={isSaving}>
                  {isSaving ? "Salvando..." : editingCustomer ? "Salvar Alterações" : "Adicionar Cliente"}
                </Button>
              </div>
            </form>
          </DialogContent>
